...
import com.busanit501.androidlabtest501.databinding.ActivityCh11Ch12MixMainBinding
import com.busanit501.androidlabtest501.databinding.ToolbarMainLayoutBinding

class Ch11_Ch12_MixMainActivity : AppCompatActivity() {
    lateinit var binding : ActivityCh11Ch12MixMainBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        binding = ActivityCh11Ch12MixMainBinding.inflate(layoutInflater)
        setContentView(binding.root)
//        setContentView(R.layout.activity_ch11_ch12_mix_main)
...
//        setSupportActionBar(binding.toolbar)
        ...
        binding.mainDrawerView.setNavigationItemSelectedListener {
                it ->
            if (it.title == "로그인") {
                Toast.makeText(this@Ch11_Ch12_MixMainActivity,"로그인 화면 이동",Toast.LENGTH_SHORT).show()
            }
            else if (it.title == "로그아웃") {
                Toast.makeText(this@Ch11_Ch12_MixMainActivity,"로그아웃 화면 이동",Toast.LENGTH_SHORT).show()
            }
            else if (it.title == "메인가기") {
                Toast.makeText(this@Ch11_Ch12_MixMainActivity,"메인가기 화면 이동",Toast.LENGTH_SHORT).show()
            }
            true
        }

        // 드로워 화면에 액션 버튼 클릭시 -> 드로워 화면에 나오게.
        toggle = ActionBarDrawerToggle(this@Ch11_Ch12_MixMainActivity,
        ...
        binding.floatingActionButton.setOnClickListener {
            ...
            Toast.makeText(this@Ch11_Ch12_MixMainActivity,"floatingActionButton 클릭됨", Toast.LENGTH_SHORT).show()
        }
...
//  override fun onOptionsItemSelected(item: MenuItem): Boolean { 추가됨.
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        if(toggle.onOptionsItemSelected(item)){
            ...
            Toast.makeText(this@Ch11_Ch12_MixMainActivity,"툴바메뉴1 클릭됨", Toast.LENGTH_SHORT).show()
            true
        }
        else if ( R.id.menu_toolbar2 == item.itemId) {
            Toast.makeText(this@Ch11_Ch12_MixMainActivity,"툴바메뉴2 클릭됨", Toast.LENGTH_SHORT).show()
            true
        }
        else if ( R.id.menu_toolbar3 == item.itemId) {
            Toast.makeText(this@Ch11_Ch12_MixMainActivity,"툴바메뉴3 클릭됨", Toast.LENGTH_SHORT).show()
            true
        }
...
//        menuInflater.inflate(R.menu.menu_main,menu)
        ...
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextSubmit(query: String?): Boolean {
                ...
                Toast.makeText(this@Ch11_Ch12_MixMainActivity,"검색어가 전송됨 : ${query}", Toast.LENGTH_SHORT).show()
                return true
            }
        })
